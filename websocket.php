...
class Websocket{
	private $socket, $is_connected = false, $is_closing = false, $last_opcode = null,
	...
	public function setUri($uri)		{	$this->socket_uri = $uri;		}
	public function upTime(){
		return ($this->is_connected)?(microtime(true)-$this->up_since):-1;
	}
	...
	public function send($payload, $opcode = 'text', $masked = true) {
		...
		if (!in_array($opcode, array_keys((array)self::$opcodes))) {
	...
	public function receive(){
		...
		$this->huge_payload = '';
		$response = null;
		while (is_null($response)/*&&((time()-$this->last_ts)<$this->options['timeout'])*/) $response = $this->receive_fragment();
		return $response;
	}
	...
	/*
		$status  http://tools.ietf.org/html/rfc6455#section-7.4
		$message A closing message, max 125 bytes.
	*/
	...
	protected function read($length) {
		...
		while (strlen($data) < $length) {
			...
			// -- wtf ? why did THAT stop everything ?
			/*
			if ($buffer === '') {
				$metadata = stream_get_meta_data($this->socket);
				throw new ConnectionException(
